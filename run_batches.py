...
def init_deforumargs(img:DeforumArgs, root:Root, motion:DeforumAnimArgs, fixed_global_seed):
    ...
    # if we run batch with a fixed seed, every combination uses that seed. 
    if fixed_global_seed is not None:
        img.seed = fixed_global_seed
    ...
    print(f"### output folder: '{img.outdir}'") # type: ignore
...
def generate_combination_name(root_conbination, motion_combination, img_combination):
    raw_name = f"{root_combination}_{motion_combination}_{img_combination}"
    ...
    return "".join([c for c in raw_name if c not in "'.{} "])
...
parser.add_argument("--fixed-global-seed", type=int, nargs='?', default=None, help="Override random seed generation with a reproduceable fixed seed every run")

args = parser.parse_args()
dry_run = args.dry_run
input_file = args.input
fixed_global_seed = args.fixed_global_seed
    ...
    for img_combination in generate_combinations(img_properties, {}, 0):
        for motion_combination in generate_combinations(motion_properties, {}, 0):
            for root_combination in generate_combinations(root_properties, {}, 0):
                ...
                print(f"### motion_instance.max_frames: {motion_instance.max_frames}")
                ...
                conbination_name = generate_combination_name(img_combination, motion_combination, root_combination)
                init_deforumargs(img_instance, root_instance, motion_instance, fixed_global_seed)

                count += 1
                if args.dry_run:
                    print(f"### Dry run: skipping rendering of {composition_file_name}")   
                else:
